...
export class TokenService {
  ...
  async generateTokens(user: User) {
    ...
    const tokens = {
      accessToken:
        'Bearer ' +
        (await this.jwtService.signAsync(payload, this.accessTokenOptions)),
      refreshToken:
        'Bearer ' +
        (await this.jwtService.signAsync(payload, this.refreshTokenOptions)),
    };
    return tokens;
  }
    ...
    try {
      ...
      return {
        payload: this.jwtService.verify(token, {
          secret: tokenVariantsSecret[tokenVariant],
        }),
        token,
      };
    } catch (e) {
